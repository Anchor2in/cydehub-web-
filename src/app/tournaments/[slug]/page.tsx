...
import { getAggregatedTournamentPostBySlug } from "@/lib/tournaments/aggregate";

type PageProps = {
  params: Promise<{ slug: string }>;
};

export default async function TournamentStoryPage({ params }: PageProps) {
  const { slug } = await params;
  const post = await getAggregatedTournamentPostBySlug(slug);
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10">
      ...
      <article className="animated-sheen rounded-3xl border border-white/10 bg-black/40 p-7 shadow-cyber-soft">
        ...
        <div className="mt-3 text-sm text-white/70">Game: {post.game}</div>

        <div className="mt-2 text-xs text-white/50">{new Date(post.starts_at).toLocaleString()}</div>
